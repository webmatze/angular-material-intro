...
function getTemplateCache() {
    return gulp.src(templates)
        .pipe(plumber())
        .pipe(templateCache({
            module: 'angular-material-intro.directives'
        }));
}
...
gulp.task('build', function() {
    return eventStream.merge(gulp.src(sourceFiles), getTemplateCache())
        .pipe(plumber())
        .pipe(concat('angular-material-intro.js'))
        .pipe(gulp.dest('./dist/'))
        .pipe(uglify())
        .pipe(rename('angular-material-intro.min.js'))
        .pipe(gulp.dest('./dist'));
});
...
gulp.task('build-stylesheets', function() {
    return gulp.src(stylesheets)
        ...
        .pipe(sass())
        .pipe(concatCss('angular-material-intro.css'))
        .pipe(gulp.dest('./dist'));
});
